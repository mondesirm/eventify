				...
				>
					{({ handleChange, handleSubmit, handleBlur, values, errors, touched }) => (
						<View style={styles.inputs}>
							{/* <InputView ref={inputs[0]} navs={[0, navs]} autoFocus type="username" label={__('form.username')} left="account-outline" value={values.username} errors={[touched.username, errors.username]} onBlur={handleBlur('username')} onChangeText={handleChange('username')} />
							<InputView ref={inputs[1]} navs={[1, navs]} type="email" label={__('form.email')} left="email-outline" value={values.email} errors={[touched.email, errors.email]} onBlur={handleBlur('email')} onChangeText={handleChange('email')} />
							<InputView ref={inputs[2]} navs={[2, navs]} type="phone" label={__('form.phone')} left="phone-outline" value={values.phone} errors={[touched.phone, errors.phone]} onBlur={handleBlur('phone')} onChangeText={handleChange('phone')} />
							<InputView ref={inputs[3]} navs={[3, navs]} type="password" label={__('form.password')} left="lock-outline" value={values.password} errors={[touched.password, errors.password]} onBlur={handleBlur('password')} onChangeText={handleChange('password')} />
							<InputView ref={inputs[4]} navs={[4, navs]} type="password" label={__('form.confirm')} left="lock-outline" value={values.confirm} errors={[touched.confirm, errors.confirm]} onBlur={handleBlur('confirm')} onChangeText={handleChange('confirm')} returnKeyType="done" /> */}

							{/* Same thing as comment above */}
							{_.map(_.keys(values), (key: keyof typeof schema.fields, i) => <InputView ref={inputs[i]} navs={[i, navs]} key={i} type={key} label={schema.fields[key].describe()?.['label']} left={schema.fields[key].describe()?.['meta']?.icon + '-outline' as any} value={values[key]} errors={[touched[key], errors[key]]} onBlur={handleBlur(key)} onChangeText={handleChange(key)} returnKeyType={i === _.keys(values).length - 1 ? 'done' : 'next'} /> )}
